		...
		if ((isset($_COOKIE[$this->id_base . '-' . $this->number]) && $this->hash_mailing_list_id($this->number) == $_COOKIE[$this->id_base . '-' . $this->number]) || false == $this->ns_mc_plugin->get_mcapi()) {
			...
			if ($this->successful_signup) {
				...
				<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" id="<?php echo $this->id_base . '_form-' . $this->number; ?>" method="post">
					...
					?>	
                                    <div class="widget-field"><label><?php echo __('First Name :', 'mailchimp-widget'); ?><input type="text" name="<?php echo $this->id_base . '_first_name'; ?>" /></label></div>
					<br />
					<?php
						}
						if ($instance['collect_last']) {
					?>	
					<div class="widget-field"><label><?php echo __('Last Name :', 'mailchimp-widget'); ?><input type="text" name="<?php echo $this->id_base . '_last_name'; ?>" /></label></div>
					<br />
					<?php	
						}
					?>
						<input type="hidden" name="ns_mc_number" value="<?php echo $this->number; ?>" />
						<div class="widget-field"><label for="<?php echo $this->id_base; ?>-email-<?php echo $this->number; ?>"><?php echo __('Email Address :', 'mailchimp-widget'); ?></label>
						<input id="<?php echo $this->id_base; ?>-email-<?php echo $this->number; ?>" type="text" name="<?php echo $this->id_base; ?>_email" /></div>
