		...
		if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) {
			foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
				$_product = $values['data'];
                                ...
                                switch ($product_type){
                                    case "pm":
                                        $quantity_desc = '<span class="quantity-desc">per metre</span>';
                                        $quant = '<span class="quantity-desc">metres</span>';
                                        break;
                                    case "pr100":
                                        $quantity_desc = '<span class="quantity-desc">per 100m reel</span>';
                                        $quant = '<span class="quantity-desc">reels</span>';
                                        break;
                                    case "pr500":
                                        $quantity_desc = '<span class="quantity-desc">per 500m reel</span>';
                                        $quant = '<span class="quantity-desc">reels</span>';
                                        break;
                                    }
					...
					<tr class = "<?php echo esc_attr( apply_filters('woocommerce_cart_table_item_class', 'cart_table_item', $values, $cart_item_key ) ); ?>">
						...
						<td class="product-price">
							<?php
								...
								echo apply_filters('woocommerce_cart_item_price_html', woocommerce_price( $product_price ), $values, $cart_item_key ). " " . $quantity_desc;
							?>
						</td>
						...
						<td class="product-quantity">
							<?php
								...
								echo apply_filters( 'woocommerce_cart_item_quantity', $product_quantity, $cart_item_key ). " " . $quant;
							?>
						</td>
